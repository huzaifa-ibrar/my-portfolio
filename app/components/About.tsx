...
import { 
  ...
  FaFileCode, FaUniversity, FaShieldAlt,
  FaDesktop, FaTools, FaNetworkWired
} from 'react-icons/fa';
import { SiKubernetes, SiJenkins, SiTerraform, SiYaml, SiCplusplus, SiAzure, SiSplunk } from 'react-icons/si';
...
const About = () => {
  ...
  const skills = [
    ...
    { name: 'Azure', icon: <SiAzure className="w-full h-full" />, category: 'devops' },
    ...
    // Security & Tools
    { name: 'DevSecOps', icon: <FaShieldAlt className="w-full h-full" />, category: 'security' },
    { name: 'Network Security', icon: <FaNetworkWired className="w-full h-full" />, category: 'security' },
    { name: 'VMware', icon: <FaDesktop className="w-full h-full" />, category: 'tools' },
    { name: 'Wireshark', icon: <FaCode className="w-full h-full" />, category: 'tools' },
    { name: 'Splunk', icon: <SiSplunk className="w-full h-full" />, category: 'tools' },
  ];
          ...
          <svg width="100%" height="100%" viewBox="0 0 100 100" preserveAspectRatio="none">
            <defs>
              ...
              <radialGradient id="glow2" cx="70%" cy="70%" r="60%" gradientUnits="userSpaceOnUse">
                <stop offset="0%" stopColor="rgba(64, 224, 208, 0.2)" />
                <stop offset="100%" stopColor="rgba(0, 0, 0, 0)" />
              </radialGradient>
            </defs>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          ...
          >
              ...
              <div className="p-8">
                ...
                <div className="space-y-6">
                  ...
                  >
                    ...
                    <p className="text-light/60 mt-2">Expected: 2026</p>
                  </motion.div>
                  ...
                  <motion.div
                    ...
                    className="mt-8 border-t border-primary/20 pt-8"
                  >
            ...
            <div className="mt-12 grid grid-cols-1 gap-6">
              {[
                {title: 'Programming Languages', content: 'Python, Java, Git, C++, SQL'},
                {title: 'DevOps & Cloud', content: 'Azure, Jenkins, Terraform, AWS, Docker, Kubernetes'},
                {title: 'Security Practices', content: 'DevSecOps, Cloud Security, Penetration Testing, Network Security'},
                {title: 'Software Tools', content: 'VMware, Wireshark, Splunk, Cisco Packet Tracer'}
              ].map((category, index) => (
                <motion.div
                  key={index}
                ...
                >
                  <h4 className="text-xl font-bold mb-3 text-primary">{category.title}</h4>
                  <p className="text-light/80">{category.content}</p>
                </motion.div>
              ))}
            </div>
          </motion.div>
                    ...
                    <motion.button
                      ...
                      className={`px-6 py-3 rounded-full text-lg font-medium transition-all duration-300 ${
                        activeCategory === tab.id 
                          ? 'bg-primary text-light shadow-[0_0_15px_rgba(64,224,208,0.5)]' 
